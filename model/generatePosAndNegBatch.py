...
class Train_dataset:
    ...
    def __getitem__(self, index):
        # sample = self.generateTrainsample(index)
        # return sample
        tmpHead = self.train2id["s"][index]  # 1*1
        tmpRelation = self.train2id["r"][index]
        tmpTail = self.train2id["o"][index]
        tmpTime = self.train2id["t"][index][3]
        ...
        #         ]
        # return [tmpHead, tmpRelation, tmpTail, tmpTime, ms.Tensor(ns, ms.int64), ms.Tensor(no, ms.int64), \
        #         ms.Tensor(s_h_r, ms.int64), ms.Tensor(s_h_e, ms.int64), ms.Tensor(s_h_t, ms.int64), \
        #         ms.Tensor(o_h_r, ms.int64), ms.Tensor(o_h_e, ms.int64), ms.Tensor(o_h_t, ms.int64), \
        #         tmpHead_d, tmpTail_d, tmpTime_m, ms.Tensor(p_s_o_r, ms.int64), \
        #         ]
        return (np.array(tmpHead), np.array(tmpRelation), np.array(tmpTail), np.array(tmpTime), \
               np.array(ns).astype(np.int32), np.array(no).astype(np.int32), np.array(s_h_r).astype(np.int32), \
               np.array(s_h_e).astype(np.int32), np.array(s_h_t).astype(np.int32), np.array(o_h_r).astype(np.int32), \
               np.array(o_h_e).astype(np.int32), np.array(o_h_t).astype(np.int32), np.array(tmpHead_d), \
               np.array(tmpTail_d), np.array(tmpTime_m), np.array(p_s_o_r).astype(np.int32))
...
class Test_dataset():
    ...
    def __getitem__(self, item):
        ...
        #         ]
        return (np.array(tmpHead), np.array(tmpRelation), np.array(tmpTail), np.array(tmpTime), \
                np.array(s_h_r).astype(np.int32), np.array(s_h_e).astype(np.int32), np.array(s_h_t).astype(np.int32), \
                np.array(o_h_r).astype(np.int32), np.array(o_h_e).astype(np.int32), np.array(o_h_t).astype(np.int32))
    ...
    def generateTestsample(self, item):
        ...
        #         ]
        return [tmpHead, tmpRelation, tmpTail, tmpTime, \
                ms.Tensor(s_h_r, ms.int32), ms.Tensor(s_h_e, ms.int32), ms.Tensor(s_h_t, ms.int32), \
                ms.Tensor(o_h_r, ms.int32), ms.Tensor(o_h_e, ms.int32), ms.Tensor(o_h_t, ms.int32), \
                ]
