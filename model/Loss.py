...
def normLoss(embeddings, dim=1):
	...
	norm = ms.ops.ReduceSum(keep_dims=True)(embeddings ** 2, axis=dim)
	return ms.ops.ReduceMean()(ms.ops.maximum(norm - ms.Parameter(ms.Tensor([1.0], ms.float32)),
                                                        ms.Parameter(ms.Tensor([0.0], ms.float32))))
def F_norm(martix):
    # norm = torch.norm(martix)
    # return torch.max(norm - autograd.Variable(floatTensor([1.0])), autograd.Variable(floatTensor([0.0])))
    norm = ms.ops.LpNorm(axis=[0, 1], p=2, keep_dims=False)(martix)
    return ms.ops.maximum(norm - ms.Parameter(ms.Tensor([1.0], dtype=ms.float32)),
                          ms.Parameter(ms.Tensor([0.0], dtype=ms.float32)))
